...
for i in range(N):
    ...
    # Convert complex roots to polynomial coefficients
    roots = desired_roots[i, :]
    poly_coeffs = np.poly(roots)  # This gives coefficients for the polynomial
    G = control.tf([1], poly_coeffs)  # Numerator=1, denominator=poly_coeffs
    desired_char_poly = poly_coeffs
    cd1 = poly_coeffs[1]
    cd0 = poly_coeffs[2]
    print("desired_char_poly:", desired_char_poly)
    kp[i] = cd1/(b)-a/b  # dummy variables - you change!
    ki[i] = cd0/b  # dummy variables - you change!
    print("kp[i]:", kp[i])
    print("ki[i]:", ki[i])
    # PI Control
    C = (kp[i] * s + ki[i]) / s
